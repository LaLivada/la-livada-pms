-- IP-ul clientului, transmis explicit.
--
-- Crearea rezervarii trece printr-o functie edge (verificarea Turnstile
-- cere o cheie secreta, iar PostgREST nu poate face apeluri HTTP). Din
-- acel moment `request.headers` nu mai contine adresa vizitatorului, ci
-- pe a functiei edge — deci limita pe IP ar fi numarat toate rezervarile
-- lumii pe aceeasi adresa.
--
-- Parametrul e de incredere fiindca, dupa mutare, singurul care poate
-- apela functia e service_role, adica functia edge, care citeste adresa
-- din propria cerere. Antetul ramane rezerva pentru cazul in care
-- parametrul lipseste.
...
-- Semnatura veche, fara p_client_ip, ar fi ramas alaturi si ar fi facut
-- apelul cu argumentele vechi ambiguu.
